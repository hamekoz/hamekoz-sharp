//From: http://varionet.wordpress.com/2007/11/29/convertir-numeros-a-letras/
...
namespace Hamekoz.Core
{
	...
	public sealed class Numalet
	{
		...
		const int UNI = 0, DIECI = 1, DECENA = 2, CENTENA = 3;
		static readonly string[,] _matriz = {
		...
		Int32 _decimales = DecimalesDefault;
		CultureInfo _cultureInfo = CultureInfo.CurrentCulture;
		String _separadorDecimalSalida = SeparadorDecimalSalidaDefault;
		Int32 _posiciones = DecimalesDefault;
		String _mascaraSalidaDecimal, _mascaraSalidaDecimalInterna = MascaraSalidaDecimalDefault;
		Boolean _esMascaraNumerica = true;
		Boolean _letraCapital = LetraCapitalDefault;
		Boolean _convertirDecimales = ConvertirDecimalesDefault;
		...
		public String SeparadorDecimalSalida {
			...
			set {
				...
				ApocoparUnoParteEntera = value.Trim ().IndexOf (" ") > 0;
			}
		}
		...
		public String MascaraSalidaDecimal {
			get {
				return !String.IsNullOrEmpty (_mascaraSalidaDecimal) ? _mascaraSalidaDecimal : "";
			}
			set {
		...
		public Boolean ConvertirDecimales {
			...
			set {
				...
				if (value) {// Si la máscara es la default, la borro
		...
		{
			return Convertir ((Decimal)Numero, _decimales, _separadorDecimalSalida, _mascaraSalidaDecimalInterna, _esMascaraNumerica, _letraCapital, _convertirDecimales, ApocoparUnoParteEntera, ApocoparUnoParteDecimal);
		}
		...
		{
			return Convertir ((Decimal)Numero, 0, _separadorDecimalSalida, _mascaraSalidaDecimalInterna, _esMascaraNumerica, _letraCapital, _convertirDecimales, ApocoparUnoParteEntera, false);
		}
		...
		static String Convertir (Decimal Numero, Int32 Decimales, String SeparadorDecimalSalida, String MascaraSalidaDecimal, Boolean EsMascaraNumerica, Boolean LetraCapital, Boolean ConvertirDecimales, Boolean ApocoparUnoParteEntera, Boolean ApocoparUnoParteDecimal)
		{
			Int64 Num;
			...
			if (Num >= 1000000000000 || Num < 0)
				throw new ArgumentException ("El número '" + Numero + "' excedió los límites del conversor: [0;1.000.000.000.000)");
			if (Num == 0)
				...
				while (Num > 0) {
					...
					if (unidadTerna != 0)
						cadTerna = _matriz [DECENA, (decenaTerna / 10)] + " y" + _matriz [UNI, unidadTerna] + cadTerna;
					else
						cadTerna += _matriz [DECENA, (decenaTerna / 10)];
					...
					case 7:
					case 9:
						cadTerna = _matriz [CENTENA, (terna / 100)] + cadTerna;
						break;
					default:
						if ((terna / 100) > 1)
							cadTerna = _matriz [UNI, (terna / 100)] + "cientos" + cadTerna;
						break;
					}
                ...
                //Acentúo 'veintidós', 'veintitrés' y 'veintiséis'
					...
					switch (iTerna) {
					case 3:
						cadTerna += Numero < 2000000 ? " millón" : " millones";
						break;
					case 2:
					case 4:
						if (terna > 0)
			...
			if (Decimales > 0) {
				...
				var EnteroDecimal = (Int32)Math.Round ((Double)(Numero - (Int64)Numero) * Math.Pow (10, Decimales), 0);
				if (ConvertirDecimales) {
					...
					Resultado.Append (EnteroDecimal + " " + MascaraSalidaDecimal);
			}
